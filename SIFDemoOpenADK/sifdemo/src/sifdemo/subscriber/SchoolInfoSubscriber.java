...
public class SchoolInfoSubscriber extends BaseSubscriber 
{
	...
	{
		// query.addCondition( SchoolDTD.SCHOOLINFO_LOCALID, ComparisonOperators.LE, "4700" );
	}
  
	@Override
    public void processEvent(SIFEvent sifEvent, Zone zone, MappingInfo mappingInfo, String consumerID) throws ADKException
    {
		doWork(consumerID); // pretend to do some work ...
	...
	private void doWork(String consumerID)
	{
		try
		{
			int delay = generator.nextInt(5)+1;
			System.out.println(consumerID+" works for "+(delay*100)+" millisecond.");
			Thread.sleep(delay * 100); // pretend to do some work ...
		}
		catch (Exception ex){}
	}
